import java.util.Scanner;

import model.Mahasiswa;
import model.Prodi;
import repository.Database;

public class KampusApp {
    public static void main(String[] arg) {
        KampusApp kampusApp = new KampusApp();
        kampusApp.showMenu();
    }

    private Scanner scanner;
    private Database db;

    public KampusApp() {
        scanner = new Scanner(System.in);
        db = new Database();
    }

    public void showMenu() {
        System.out.println("---------------------------------");
        System.out.println("Selamat Datang di Aplikasi Kampus");
        System.out.println("Pilihan Menu:");
        System.out.println("1 -> Tambah Data Mahasiswa");
        System.out.println("2 -> Ubah Data Mahasiswa");
        System.out.println("3 -> Hapus Data Mahasiswa");
        System.out.println("4 -> Cari Data Mahasiswa");
        System.out.println("5 -> Tampil Semua Data Mahasiswa");
        System.out.println("6 -> Tambah Data Prodi");
        System.out.println("0 -> Keluar Aplikasi");
        System.out.print("Silahkan masukan menu yang dipilih: ");
        int menuYangDipilih = scanner.nextInt();
        scanner.nextLine();
        switch(menuYangDipilih) {
            case 1:
            menuTambah();
            break;
            case 2:
            menuUbah();
            break;
            case 3:
            menuHapus();
            break;
            case 4:
            menuCari();
            break;
            case 5:
            menuTampilSemua();
            break;
            case 6:
            menuTambahProdi();
            break;
            default: {
                System.out.print("* Terimakasih sudah menggunakan Aplikasi Kampus *");
                System.exit(0);
            }
            break;
        }
        scanner.close();
    }

    public void menuTambah() {
        System.out.println("----- Menu Tambah Mahasiswa -----");
        System.out.print("Masukkan NIM: ");
        String nim = scanner.nextLine();
        System.out.print("Masukkan Nama: ");
        String nama = scanner.nextLine();
        System.out.print("Masukkan ID Prodi: ");
        String idProdi = scanner.nextLine();
    
        Prodi prodi = db.getProdiById(idProdi);
        if (prodi != null) {
            db.mahasiswaTbl.create(nim, nama, idProdi);
            System.out.println("Mahasiswa berhasil ditambahkan.");
        } else {
            System.out.println("Prodi dengan ID " + idProdi + " tidak ditemukan. Silakan tambahkan Prodi terlebih dahulu.");
        }
    
        System.out.println("Tekan Enter untuk melanjutkan...");
        scanner.nextLine();
        showMenu();
    }
    

    public void menuUbah() {
        System.out.println("----- Menu Ubah Data Mahasiswa -----");
        System.out.print("Masukan NIM Mahasiswa yang akan diubah: ");
        String nimLama = scanner.nextLine();
    
        Mahasiswa mahasiswaToUpdate = db.mahasiswaTbl.getMahasiswaByNim(nimLama);
    
        if (mahasiswaToUpdate != null) {
            System.out.println("Data Mahasiswa Sebelumnya:");
            System.out.println("NIM: " + mahasiswaToUpdate.getNim());
            System.out.println("Nama: " + mahasiswaToUpdate.getNama());
            System.out.println("ID Prodi: " + mahasiswaToUpdate.getProdi().getId());
            System.out.println("Nama Prodi: " + mahasiswaToUpdate.getProdi().getNama());
    
            System.out.print("Masukkan NIM Baru: ");
            String nimBaru = scanner.nextLine();
            System.out.print("Masukkan Nama Baru: ");
            String namaBaru = scanner.nextLine();
            System.out.print("Masukkan ID Prodi Baru: ");
            String idProdiBaru = scanner.nextLine();
    
            Prodi prodiBaru = db.getProdiById(idProdiBaru);
            if (prodiBaru != null) {
                Mahasiswa mahasiswaBaru = new Mahasiswa(nimBaru, namaBaru, prodiBaru);
                db.mahasiswaTbl.update(nimLama, mahasiswaBaru);
    
                System.out.println("Data Mahasiswa berhasil diubah.");
            } else {
                System.out.println("Prodi dengan ID " + idProdiBaru + " tidak ditemukan.");
            }
        } else {
            System.out.println("Mahasiswa dengan NIM " + nimLama + " tidak ditemukan.");
        }
    
        System.out.println("Tekan Enter untuk melanjutkan...");
        scanner.nextLine();
        showMenu();
    }
    
    

    public void menuCari(){
        System.out.println("----- Menu Cari Data Mahasiswa -----");
        System.out.print("Masukkan NIM: ");
        String nim = scanner.nextLine();
        Mahasiswa mahasiswa = db.mahasiswaTbl.getMahasiswaByNim(nim);
        if (mahasiswa == null) {
            System.out.println("* NIM tidak ditemukan *");
        } else {
            Prodi prodi = mahasiswa.getProdi();
            System.out.println("* NIM: " + mahasiswa.getNim() + " *");
            System.out.println("* Nama: " + mahasiswa.getNama() + " *");
            System.out.println("* ID Prodi: " + prodi.getId() + " *");
            System.out.println("* Nama Prodi: " + prodi.getNama() + " *");
        }
        System.out.println("Tekan Enter untuk melanjutkan...");
        scanner.nextLine();
        showMenu();
    }
    
    public void menuTampilSemua() {
        System.out.println("----- Semua Data Mahasiswa -----");
        for (Mahasiswa mahasiswa : db.getTables().dataMahasiswa) {
            Prodi prodi = mahasiswa.getProdi();
            System.out.println("* NIM: " + mahasiswa.getNim() + " *");
            System.out.println("* Nama: " + mahasiswa.getNama() + " *");
            System.out.println("* ID Prodi: " + prodi.getId() + " *");
            System.out.println("* Nama Prodi: " + prodi.getNama() + " *");
            System.out.println("-----------------------------");
        }
        System.out.println("Tekan Enter untuk melanjutkan...");
        scanner.nextLine();
        showMenu();
    }
    

    public void menuHapus() {
        System.out.println("----- Menu Hapus Data Mahasiswa -----");
        System.out.print("Masukan NIM: ");
        String nim = scanner.nextLine();
        db.mahasiswaTbl.delete(nim);
        System.out.println("Tekan Enter untuk melanjutkan...");
        scanner.nextLine();
        showMenu();
    }

    public void menuTambahProdi() {
        System.out.println("----- Menu Tambah Prodi -----");
        System.out.print("Masukkan ID Prodi: ");
        String idProdi = scanner.nextLine();
    
        if (db.getProdiById(idProdi) != null) {
            System.out.println("Prodi dengan ID " + idProdi + " sudah ada.");
        } else {
            System.out.print("Masukkan Nama Prodi: ");
            String namaProdi = scanner.nextLine();
    
            db.getTables().dataProdi.add(new Prodi(idProdi, namaProdi));
            db.commit();
            System.out.println("Prodi dengan ID " + idProdi + " berhasil ditambahkan.");
        }
    
        System.out.println("Tekan Enter untuk melanjutkan...");
        scanner.nextLine();
        showMenu();
    }

}